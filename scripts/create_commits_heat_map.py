...
def create_commits_heatmap(df, date_field, period="M"):
    """
    Create a heatmap showing commit intensity across repositories and time periods.
    ...
    heatmap_data = (
        df.groupby(["repo_full_name", "time_period"])
        .size()
    ...
    pivot_data = heatmap_data.pivot(
        index="repo_full_name", columns="time_period", values="commit_count"
    )
    ...
    cbar.set_label("Number of Commits", fontsize=FONT_SIZES["legend"])
...
def create_commits_heatmap_quarterly(df, date_field):
    """
    Create a quarterly heatmap showing commit intensity.
    """
    return create_commits_heatmap(df, date_field, period="Q")


def create_commits_heatmap_monthly(df, date_field):
    """
    Create a monthly heatmap showing commit intensity.
    """
    return create_commits_heatmap(df, date_field, period="M")


def create_commits_heatmap_yearly(df, date_field):
    """
    Create a yearly heatmap showing commit intensity.
    """
    return create_commits_heatmap(df, date_field, period="Y")
...
def main():
    ...
    fig1 = create_commits_heatmap_quarterly(df, "author_date")
    fig1.savefig(
        outputDir / "commits_heatmap_quarterly.png", dpi=300, bbox_inches="tight"
    )
    fig1.savefig(
        figuresDir / "rq1_commits_heatmap_quarterly.pdf",
        bbox_inches="tight",
        format="pdf",
    )
    plt.close(fig1)
    ...
    fig2 = create_commits_heatmap_monthly(df, "author_date")
    fig2.savefig(
        outputDir / "commits_heatmap_monthly.png", dpi=300, bbox_inches="tight"
    )
    fig2.savefig(
        figuresDir / "rq1_commits_heatmap_monthly.pdf",
        bbox_inches="tight",
        format="pdf",
    )
    plt.close(fig2)
    ...
    fig3 = create_commits_heatmap_yearly(df, "author_date")
    fig3.savefig(outputDir / "commits_heatmap_yearly.png", dpi=300, bbox_inches="tight")
    fig3.savefig(
        figuresDir / "rq1_commits_heatmap_yearly.pdf", bbox_inches="tight", format="pdf"
    )
    plt.close(fig3)
